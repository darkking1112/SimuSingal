...
本次补充：① 使用官方 `sigmf==1.11.1` 实现生成信号的 SigMF 双文件导出和导入，详见 §4.1；该功能已超出此前“交换格式候选”的规划状态。② 能量检测与三参数估计、可选 AI（ONNX Runtime）检测、A09 六类调制识别以及“算法对比”页与离线报告指标表均已实现并接线，接口契约、真值评分口径与实测指标见 §4.6～§4.9。③ 2026-09-13 新增**跳频逐跳参数估计**（算法 `hop_track_v1`、契约 `fh_hops_v1`，见 §4.10）：在同一份 STFT 上做逐帧门限游程与时频脊线跟踪，给出跳频点、跳时刻、驻留时间、跳速与占空比，并只读复用 §4.6 的会话级结果作为基线，**不改动**冻结的 `detect_result_v1`。④ 2026-09-13 在同一契约上再落地**逐跳 AI 通路** `ml_detect_hops`（CLI `ml-detect-hops`，模型清单须声明 `label_semantics = per_hop_v1`，见 §4.7 与 §4.10）：网络只出逐跳候选框，驻留/功率/带宽/SNR 仍由 `hop_track_v1` 的测量逻辑在原始 PSD 上重测，因此 AI 与传统逐跳可同口径并排评分。区间标注、教学测评与功率标定元数据仍未实现。

基础代码已实现 NPY/CSV/交织 IQ 二进制导入（二进制须显式指定数据类型与字节序）、数学双音演示（入口在“IQ 信号生成”页，点数 = 页内采样率 × 持续时间）、均值/RMS/峰值、PSD/STFT、四类图形（波形、功率谱、时频图或星座图、瀑布图）、资产备注、运行历史、报告、原生复制插件，以及信号 IQ 生成器（AM/FM/SSB/2ASK/QPSK/16QAM/64QAM/跳频 FH-2FSK 遥控、FH-OFDM 图传，多信号合成与带限噪声，作为检测、参数估计与调制识别算法的测试信号源）。分析提供一键概览与可调速实时播放两种方式；数字或模拟的判定为启发式，可在界面上手动纠正。检测与参数估计（能量基线）、跳频逐跳参数估计（传统 `hop_track_v1` 与 AI `ml_detect_hops` 两条通路）、AI 检测（ONNX Runtime，可选）、A09 六类调制识别（线性基线随包分发）、双路径并排对比与 HTML 报告指标表均已实现并冻结契约；**区间标注、教学测评、AM 解调后的消息侧带与功率标定元数据仍未实现**。逐文件说明及测试记录见[基础工程实现与文件说明](基础工程实现与文件说明.md)；工作区容量盘点与安全清理见[数据管理页面](数据管理页面.md)；下文为完整目标方案。
...
| A09+ | （超出原文）原始 IQ 输入的分类分支 | `iq_waveform_v1` / `amc_iq_classify_v1`（§4.8.1） | 契约硬门禁、窗口长度拒绝、导出忠实性；**准确率不作结论** |
...
**当前进度（详细接口与实测数据见 §4.6～§4.10）：**

| 条目 | 现状 | 说明 |
| --- | --- | --- |
| A03 | 已实现 | 能量检测 + 中心频率/占用带宽/起止时间/带内信噪比 + 跳频逐跳参数估计 + A09 六类调制识别，界面与 CLI 同源 |
| A04 | 已实现（部分） | “算法对比”页可并排展示传统基线与 AI 检测、预测与真值；耗时史仍只在运行记录里 |
| A07 | 部分实现 | 中心频率、占用带宽、带内信噪比已有误差口径；**符号速率未做独立估计** |
| A09 | 已实现 | 六类字典、传统与模型两路结果；另增原始 IQ 通路（§4.8.1，类别字典由清单声明）；性能合格门限仍为待确认项 |
| A05 / A06 / A08 / A10 / A11 | 尚未实现 | 人工识别训练、双平台实测、两万条规模、72 小时长稳与交付签收均未开始 |

界面侧另有**数据管理**页（口径与规则见[数据管理页面](数据管理页面.md)）：按 `assets/`、`runs/`、`jobs/`、
`exports/`、`catalog.sqlite3` 分别统计占用，可把 `training/data` 等外部目录加为只统计的额外目录；核对索引与
磁盘的一致性（未入库文件、入库但文件缺失、未入库运行目录、`.tmp` 残留、源资产已删除）；并在“预览 → 勾选 →
二次确认”的门禁下清理**过期任务**与**无主文件**，已入库的资产、运行目录与 `exports/` 永不可删，运行中任务
一律跳过。该页扫描与导出都是只读的（不写运行记录），清理会追加 `maintenance.log` 审计记录。
...
频谱的频率范围由“频率显示”选择统一控制，数据分析、信号检测与跳频参数三个页面都提供该选项，取值与语义一致：`自动` 按频谱是否关于 0 Hz 镜像判断（实数记录的正负半轴互为镜像，只显示非负频率；复数 IQ 保留双边），`双边` 恒为 −fs/2～+fs/2，`仅正频率` 恒为 0～fs/2。同一页面内的平均功率谱密度曲线、时频图与检测框（逐跳框）必须共用同一频率子集，否则框与谱线会错位。时频图横轴为频率、纵轴为时间：图像数组按“行 = 时间、列 = 频率”交给 row-major 的 `ImageItem`，与 `setRect` 给出的频率跨度/时间跨度一致；写成转置矩阵会把时频图旋转 90°，在频率轴上显示出并不存在的负频率电平。
...
算法细节（思路、公式、流程图、输入输出参数、设计局限与可改进方向、参考文献）见[信号检测：传统能量检测](algorithms/信号检测_传统能量检测.md)。

几个必须写明的物理约定：

* **AM 的双边带结构。** 含载波的调幅信号在频谱上是载波峰加对称的上下边带，能量检测得到的占用带宽约为消息带宽的两倍；报告与表格给出的是**能量占用带**（并对齐其中心），不把它当成消息带宽，也不把载波峰单独当作中心频率，两者差异在结果里显式保留。
* **FH 会话合并。** 跳频信号按“**一段会话一个检测实例**”合并：同一载波上时间相邻且频率相邻的子带归并为一个 `hopping` 目标，记录子带数、跳频范围与总时长；合并判据为时间间隔 ≤ 3×最大子带带宽且时间重叠 ≤ 较短者时长的 20%，阈值写入结果配置便于复核。需要强调的是：**`sub_bands` 是会话合并次数，不是跳数**，而且相邻跳频信道之间的缝小于一个频点时，门限掩膜与闭运算会把它们连成一条连续带（实测资产 `c801d634` 得到 `hopping=false`、`sub_bands=1`），所以 `hopping` 也不能当“这是不是跳频信号”的判据。逐跳信息（跳频点、跳时刻、驻留、跳速）由 §4.10 的另一条通路给出。
...
同一输入上还有**逐跳 AI 通路** `ml-detect-hops`（算法标识 `ml_detect_hops:<模型 id>`，契约 `fh_hops_v1`）：它只接受清单声明 `label_semantics = per_hop_v1` 的模型（一跳一个框，数据集用 `build_dataset.py --labels hop` 构建），在训练侧有标签量化守卫（最小标签边不得小于输出网格步长、单样本框数不得超过 `--max-boxes`），避免标签小于一格时被静默丢弃。推理时网络只给出频带与粗时间，驻留/功率/带宽/SNR 全部由 §4.10 的同一套测量与会话归并逻辑在**原始 PSD** 上重测，因此两种逐跳结果的物理量可直接比大小；同一次调用默认附带传统逐跳基线（`summary.traditional`）与会话级基线（`summary.baseline`），报告里给出 AI 逐跳/传统逐跳/会话级三列指标。每条 AI 逐跳明细额外带 `model_confidence`（网络分数，**不是概率、未标定**）供追溯；清单语义不匹配时在推理前直接报错，不用会话级模型冒充逐跳模型。

算法细节（时频图构造、坐标契约、框解码、同口径带内辐射量、设计局限与可改进方向、参考文献）见[信号检测：AI 时频图检测](algorithms/信号检测_AI时频图检测.md)。
...
算法细节见[调制识别：传统特征与启发式判定](algorithms/调制识别_传统特征与启发式判定.md)（34 维特征定义与人工判据）与[调制识别：AI 特征学习](algorithms/调制识别_AI特征学习.md)（线性判别与 Transformer/ONNX 分支、温度标定、设计局限与可改进方向、参考文献）。两条路径共用同一份特征契约，差别只在判别层，因此可同口径比较。

#### 4.8.1 原始 IQ 通路（已实现，`iq_waveform_v1` / `amc_iq_classify_v1`）

在同一页面上改选一个**原始 IQ 清单**即可走第二条通路：不提取 34 维特征，而是把“搬移到零频 → 抽取到与带宽匹配的分析率 → 取中一段定长窗口 → 单位 RMS 归一化”后的 `(2, N)` 复基带波形直接交给 CNN/TCN 分类器，让网络自己学调制特征。输入契约 `iq_waveform_v1`（通道排布 `iq_channels_first_v1`、归一化 `unit_rms`、`64 ≤ N ≤ 65536`、默认 1024），结果契约 `amc_iq_classify_v1`。

与前一条通路的关系：

* **前端同一份实现。** 混频、抗混叠滤波与抽取复用 §4.8 的同一份代码（抽取比 `samples_per_band = 8.0`、低通抽头 `lowpass_taps = 65`），因此同一带宽下两条通路的分析率与滤波器完全一致，差别只在“交给判别器的东西”。
* **契约硬门禁，不互相兼容。** 输入契约是清单必填项：特征清单（`amc_feature_vector_v1`）交给 `amc-iq-classify` 会被拒绝，IQ 清单交给 `amc-classify` 同样被拒绝；窗口长度必须与清单 `input.samples` 一致，短于 64 点或长于 65536 点直接报错——**短窗口不补零**，因为补零会让“无有效信号”的输入也能产出高置信度结果。
* **类别字典由模型声明**（`a09` 六类或 `custom` 自定义，≤ 64 类），不再全局冻结，因此结果里必须原样带上 `class_set` 与 `labels`，不允许在展示层丢掉；跨模型的分数不保证可比。
* **无传统对照。** 这条通路没有确定性特征，因此不提供 §4.8 的数字/模拟、恒包络等启发式对照行，界面对应区域显示“不适用”而不是留空。
* **同样的诚实性出口。** 结果里的 `pending` 同样列出“识别准确率的合格门限尚未确认”，低信噪比或最高类概率偏低时标“仅供参考”；`snr_estimate_db` 是带内信噪比粗估，只作上下文提示、不参与判别。

入口：界面“调制识别”页（选中 `iq_waveform_v1` 清单时自动切换分支），或 CLI

```bash
signal-analysis amc-iq-classify ASSET_ID --model iq_manifest.json
signal-analysis amc-iq-classify ASSET_ID --model iq_manifest.json --offset-hz 0 --bandwidth-hz 30000 --threads 4
signal-analysis amc-iq-manifest onnx/iq.onnx iq_manifest.json --id iq-cnn-v1 --version 0.1.0 --samples 1024
```

训练侧（`training/`，不随 wheel 分发）：`build_iq_dataset.py` 用本项目生成器把随机场景转成定长 IQ 窗口数据集（每个窗口直接调用推理端 `iq_waveform` 生成，标签是生成器已知的调制样式，按类内位置分层划分，同种子同参数逐字节可复现；凑不满窗口的场景重抽而不是补零），`train_iq.py` 训练 `IQCNN`（步长卷积）/ `IQTCN`（膨胀因果卷积）并导出 ONNX（softmax 在图内、按 batch=1 导出）与清单，`verify_iq.py` 做契约、确定性场景端到端、重复推理一致与数据集独立验证（退出码区分通过 / 失败 / 缺 onnxruntime）。可选地把 TorchSig（MIT）作为补充数据源：`build_torchsig.py` 把它转成本项目自描述的 `torchsig_bundle_v1`，`build_iq_dataset.py --torchsig-bundle --torchsig-map` 按**显式类名映射**把其中的信号并入同一份数据集（未映射的类名原样记入 `unmapped_classes` 并跳过、映射目标非法则报错），样本用 `source` 字段区分来源；bundle 与其生成物只落本地目录，不随产品分发，训练侧也不依赖 TorchSig。

实测只报原始数字：冒烟规模（每类 24 条、窗口 512 点）144 训练 / 36 验证，验证准确率 0.5833、宏平均 F1 0.5727，ONNX 入口与 torch 路径一致；该规模下训练集内 0.97 对验证 0.58，足以说明**样本量远不够**，因此这些数字只用于证明链路与导出忠实，**不作为精度声明**，也不设通过判定。

算法细节（契约、前端口径、两个基线、数据集与标签语义、实测、设计局限与可改进方向）见[调制识别：AI 特征学习](algorithms/调制识别_AI特征学习.md) §7。
...
离线报告（HTML）除原始 JSON 外，按结果类型补一张指标表：检测类结果给“检测结果 / 传统基线”两列（无真值时不生成表），调制识别结果给预测字段表、真值命中表与“尚未确认项”列表；跳频逐跳结果给“AI 逐跳口径 / 传统逐跳口径 / 会话口径（能量检测基线）”三列（AI 那一路缺模型或其清单却用时少一列并在说明里写清原因）。缺值统一显示 `--`，**不以 0 或空串伪装成功**；所有文本经过转义，可直接作为交付物。

### 4.10 跳频逐跳参数估计（已实现，`hop_track_v1`）

界面“跳频参数”页与 CLI `detect-hops` 在**同一份时频图**上做逐帧游程提取与时频脊线跟踪，把 §4.6 合并成一条会话记录的跳频信号拆回逐跳：输出每一跳的中心频率、占用带宽、起跳/结束时刻、驻留时长、功率与带内信噪比，以及会话级的跳频点集合、跳速、占空比与信道间隔；结果契约 `fh_hops_v1` 冻结，GUI、CLI、报告三处共用，并在同一次调用里只读复用一遍 §4.6 的能量检测结果作为会话级基线（`summary.baseline`），两种粒度可并排对照。

诚实性出口：结果里的 `resolvable` 与 `reason` 明确回答“这份记录的时间分辨率是否够支撑逐跳结论”（一跳至少 4 个 STFT 帧是硬下限）；不满足时直说，不把融合后的稀疏结果包装成完整逐跳结果。逐跳真值只在生成器产出且有跳频样式时存在，`evaluation.hop_truth` 从生成器摘要折算，未记录生成摘要的旧资产返回 `available=false` 并说明原因，**不拿其它样式的真值硬套**。

算法细节（思路、公式、流程图、输入输出参数、实测结果、设计局限与参考文献）见[信号检测：跳频逐跳参数估计](algorithms/信号检测_跳频逐跳参数估计.md)；与会话级口径的差异见 4.6 与[信号检测：传统能量检测](algorithms/信号检测_传统能量检测.md) §6 第 7 条。

同一契约还有 **AI 逐跳通路** `ml-detect-hops`（见 §4.7 与[信号检测：AI 时频图检测](algorithms/信号检测_AI时频图检测.md) §7）：它把本文“逐帧游程 + 频点链跳”的判决换成网络给出的逐跳候选框，但**框之后的每一段都复用本节的代码**（驻留/功率/带宽/SNR 重测与会话归并），输出同为 `fh_hops_v1`。因此两条逐跳通路可以直接并排看：同一次调用里可同时得到 AI 逐跳、传统逐跳（`summary.traditional`）与会话级（`summary.baseline`）三套结果，报告里三列指标并列；AI 通路的逐跳明细多 `model_confidence` / `model_label` 两个键（网络分数），传统通路没有这两个键，界面与报告按“不适用”显示。
