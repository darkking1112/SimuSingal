...
建议实现以 Python 为主的离线桌面分析平台，完成文件导入、科学数据可视化、分析算法接入、结果对比、标注管理和教学测评。业务层、任务层和算法层分离，保证数据及运行结果可以追溯。
...
| A05 | 随机抽取数据进行人工识别、判分 | 教学测评模块 | 抽题记录、隐藏答案、作答及判分记录 |
...
| A12（用户新增） | 用户 DLL/SO 算法接入 | 原生适配器和工作进程 | 各目标平台加载与运行参考样例 |
| A13（用户新增派生） | 原生插件故障隔离 | 工作进程监控 | 超时、ABI 错误、本机崩溃不使主程序退出 |
| A14（用户新增） | 保留接口并提供示例 | C ABI、SDK、接入手册 | 用户可独立编译并接入示例 |
| A15（用户新增派生） | 打包后仍可扩展算法 | 外置插件目录和版本管理 | 新增插件无需重打主程序 |
| C01～C03（用户新增） | 核心二进制化、行为一致及源码交付 | 核心扩展构建与发布 | 无核心源码环境运行、双模式回归和独立源码包 |

A09 的六类按原文主体采用 FM、SSB、2ASK、QPSK、16QAM、64QAM。原文数据库示例另出现 AM；详细设计时统一类别字典，避免把示例值误认为新增必验类别。

### 1.2 设计基线与待确认内容

- **设计建议：** 第一条交付链路为离线文件导入、分析、展示、归档。保留设备适配接口；设备直连需协议、SDK、测试机和吞吐量要求，不能以本地文件回放替代验收。
- **原文要求：** 单机、Windows 与麒麟；原文配置包括 16 核 1.9 GHz、内存 16 GB 以上、600 GB 磁盘、显存 4 GB 以上及 1920×1080 显示。
- **待确认：** 操作系统具体版本、处理器架构、GPU 型号、文件大小分布、数据到达方式、模型权重和参考标签。
- **待确认：** 识别准确率、检测误报漏报、参数误差和处理速度的合格门限。本文不新增“准确率 95%”之类无依据承诺。
...
采用 Python 对象与消息队列进行本机调用，首期无需 HTTP 服务。大数组通过只读文件引用或受控共享内存交接，小型状态和结果通过结构化消息交接，避免反复序列化大数组。
...
| 图表 | PyQtGraph；静态报告可用 Matplotlib | 绘图数据按视口裁剪，不加载全量历史数据 |
...
原文的 ResCNN-BiLSTM-Attention 作为待接入模型条目保留。模型结构、权重来源、适用数据范围和性能应单独评审；不能用模型名称代替已完成的模型资产。
...
SDK 设计和示例范围见[补充方案](核心模块二进制化与原生插件接口方案.md)。当前[通用 float32 数据复制示例](../examples/native_plugin/README.md)已通过 `plugins.py`、`tasks.py` 接入工作台，支持清单校验、子进程调用和派生资产保存；正式算法生命周期 SDK 仍需开发。
...
机器模型评估与人员答题是两套独立流程：前者输出模型评估报告，后者输出试卷、作答与成绩。

模型评估应固定数据清单和版本，避免同源文件的相邻片段同时进入用于模型开发和独立验收的数据集合。标签来自参考来源或人工复核，模型预测不能自动充当真值。

教学测评按已配置规则抽题，保留随机种子和题目快照。提交前隐藏答案、带答案的文件名、标注图层和自动预测；提交后按规则显示判分。教学界面的隐藏不等于对具有本机文件权限的使用者提供正式考试安全隔离，如有正式考试要求需另行设计。
...
| ExerciseAttempt | 试卷 ID、题目快照、答案、提交时间、评分规则版本、得分 |
...
原文的五张核心表继续保留业务含义，但增加 `analysis_run`、`signal_segment`、`model_artifact`、`dataset_version`、`exercise_session` 和 `exercise_attempt`。
...
**建议验证目标，非原文承诺：** 在约定的 20,000 条元数据测试库上，常用条件首屏查询 P95 不超过 1 秒；用户点击后 200 ms 内显示任务受理状态。算法完成时延、绘图时延和文件大小相关，应在目标机上建立独立基准后设定门限。
...
原文列出合同后 10 日完成设计、120 日初样、140 日正样、150 日用户试用，并要求 2026-11-30 前交付。若 2026-09-06 才从零启动，距该日期为 85 天，原 150 日安排无法直接成立。必须核对签约时间及既有代码、数据、模型和测试进展。

下表为建议的约 12 周压缩安排，前提是现有模型可交接、样本及时提供、目标机首周到位、第三方测试提前预约；不满足这些条件应重新确定交期。

| 阶段 | 建议窗口 | 产出与退出条件 |
| --- | --- | --- |
| 需求与兼容性验证 | 第 1～2 周 | 格式样例、平台矩阵、性能口径、模型资产清单 |
| 数据及桌面主链路 | 第 3～4 周 | 导入、数据库、两种图形、任务管理可演示 |
| 分析适配及结果管理 | 第 5～7 周 | 各插件可运行、版本可追溯、对比报告 |
| 标注与教学测评 | 第 6～8 周，与上阶段由不同职责人员推进 | 题库、作答、判分、记录导出 |
| 集成及平台验证 | 第 9～10 周 | 容量与 72 小时稳定性测试、目标机安装 |
| 第三方测试及交付 | 第 11～12 周 | 缺陷闭环、验收包、源码、文档与培训材料 |

建议投入：技术负责人 1 人、Python 桌面/数据开发 2 人、算法适配与验证 1 人、测试及部署 1 人，约 5 人；这是软件平台估算，不含从零研制模型、采购硬件及未知设备协议逆向工作。

V1.1 新增核心编译与用户原生 SDK 工作包：第 1～2 周验证核心编译链路并形成 ABI 草案，第 3～4 周建立原生工作进程，第 5～7 周开发 SDK、参考用例及结果映射，第 9～10 周覆盖冻结应用和原生故障测试。此分配说明应前置的工作，不代表原 5 人/12 周估算已包含全部新增成本；需依据用户库及目标平台重新核算。

原文交付要求为软件含源代码、需求规格说明书、软件测试说明、第三方测试报告、验收测试报告、使用说明书；交付地点为陕西西安。另建议交付依赖锁文件、构建脚本、数据库迁移脚本、数据及模型版本清单。原文两年质保及服务时效属于合同服务条款，本方案不修改。

新增交付要求为核心扩展及构建配置、正式 C 头文件、SDK 版本说明、DLL/SO 源码示例、平台构建说明、Python 调用示例和插件接入验证记录。示例源码公开给接入者；自研核心源码按原合同约定交付。

## 10. 评审待办

1. 确定 Windows 与麒麟具体版本、CPU 架构和可用测试机。
2. 确定每种文件格式的说明、参考样本、标定信息与最大文件规模。
3. 确定设备直连是否属于本期，以及 SDK、协议和实验设备的提供时间。
4. 明确六类字典、模型资产交接、独立参考标签及数据使用范围。
5. 固定各参数定义、适用条件、误差门限、识别及检测验收指标。
6. 核对合同日期、已完成成果和第三方测试排期，重算到交付日的关键路径。
7. 提供用户 DLL/SO 的头文件、库文件、架构、运行库依赖和参考输入输出，确定标准 ABI 或适配范围。
8. 确定核心编译模块清单及源码包交付形式；确认有状态插件、输出类型和并发要求以冻结 SDK。
