...
格式建议支持“有明确元数据的交织 IQ 二进制”和 NPY；SigMF 可作为交换格式候选。不能依靠 `.bin`、`.dat` 扩展名推断采样率、I/Q 排列、量化比例或字节序。SigMF 提供采样数据与 JSON 元数据的组织方式。[SigMF 官方规范](https://sigmf.org/)。
...
**噪声功率计算：** 背景噪声为带宽 $B_n$ 的复带限高斯噪声（默认 $B_n=f_s$，即全带；范围 $0<B_n\leq f_s$）。有信号时按相对**最强目标功率信号**的 SNR 折算：

$$
P_{\mathrm{noise}}=\frac{\max_k 10^{P_k/10}}{10^{\mathrm{SNR}/10}}
$$

其中 $P_k$ 为各信号的目标功率（dBFS），SNR 默认 20 dB、范围 -60～120 dB。纯噪声（无信号）时直接按指定平均功率：

$$
P_{\mathrm{noise}}=10^{P_{\mathrm{noise,dBFS}}/10}
$$

（默认 -20 dBFS）。噪声以单位功率带限噪声乘以 $\sqrt{P_{\mathrm{noise}}}$ 叠加进记录，摘要记录噪声带宽、折算后的 `power_dbfs` 与 `snr_db`。
