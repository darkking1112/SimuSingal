...
### 本项目的落地方式：把 TorchSig 隔离在“生成 → bundle”边界之后

本仓库把 TorchSig 当作**可选的补充数据源**，而不是训练底座：产品运行不需要它，训练侧读端也不需要它
（`training/ingest_torchsig.py`、`build_iq_dataset.py` 只用 NumPy）。中间接口是一个自描述格式
`torchsig_bundle_v1`——`manifest.json` 加每条记录的 `iq.npy` 与 `meta.json`，由 `training/torchsig_bundle.py`
读写：

1. `training/build_torchsig.py` 调 TorchSig 生成 IQ 与**实例级**元数据（每条记录保留自己的采样率、
   中心频率、带宽、SNR 与信号实例列表），写成 bundle。**这是唯一需要安装 TorchSig 的环节。**
2. 之后分成两条消费路径，都不再依赖 TorchSig：
   * 检测数据集：`ingest_torchsig.py --labels session` 把实例转成 `tf_image_v1` 时频图与
     `normalized_boxes_v1` 框；
   * 分类数据集：`build_iq_dataset.py --torchsig-bundle --torchsig-map` 按**显式类名映射**
     把信号并入 `iq_waveform_v1` 数据集，与项目生成器的样本混在一份数据里，用 `source` 字段区分来源。

这样切分的理由是 §01 的“先定义任务”和本节的图意：**数据生成器必须可以整体替换，而任务契约、划分与
数据集卡必须稳定**。TorchSig 的版本、类别表与 API 都会变，把它们隔离在一条边界后面，版本升级、
许可复核和类别口径调整都只影响那一个脚本，训练与推理代码不受影响。具体做法：

* **类别语义不继承**：TorchSig 的“信号实例/调制族”与项目的跳频会话、A09 六类不是一套东西，
  映射只能**显式**给出（`--torchsig-map`）；未映射到的类名原样记进数据集的 `unmapped_classes`
  并跳过该记录，映射目标不在类别字典内则直接报错——不猜、不兜底。
* **逐跳标签不适用**：TorchSig 没有跳频族，检测侧只允许会话级标签，用 `--labels hop` 会直接失败，
  而不是产出一份语义错误的逐跳数据集。
* **许可与分发**：TorchSig（MIT）与其生成物都只落本地目录，不随产品发行（见 §07、
  [调制识别 AI 特征学习](../algorithms/调制识别_AI特征学习.md) §7.4）。
* **可复现**：bundle 记录生成参数与种子，两条消费路径都产出带划分与来源统计的数据集卡；
  训练工具链的回归测试**不需要安装 TorchSig** 即可跑（读端行为由本项目自己的格式模块定义）。
...
本仓库已按上述路径落地两条任务线，其算法思路、公式、流程图、输入输出参数、设计局限与参考文献见 `docs/algorithms/` 下五篇设计文档：[传统能量检测](../algorithms/信号检测_传统能量检测.md) 与 [AI 时频图检测](../algorithms/信号检测_AI时频图检测.md)、[跳频逐跳参数估计](../algorithms/信号检测_跳频逐跳参数估计.md)（检测的会话级与逐跳两种粒度）、[传统特征与启发式判定](../algorithms/调制识别_传统特征与启发式判定.md) 与 [AI 特征学习](../algorithms/调制识别_AI特征学习.md)。其中 AI 路径只负责判决，中心频率、带宽与带内信噪比仍由与能量路径同一份物理量测给出，避免把网络输出直接当作辐射参数。

## 11 术语速查

下表为本文反复出现的缩写与术语，正文首次出现处一般给出全称，后续统一用缩写。
...
编辑说明：已修正“YOLO 被序列模型替代”“STFT 必然丢失相位”“检测框直接给出全部射频参数”等容易误解的表述。全文图示为解释性绘制；模型选择矩阵是工程建议，文中数字示例均不是实测性能。排序整理：把“术语速查”提升为第 11 章，论文与项目参考保留为末章（第 12 章），与 Markdown、HTML 两侧一致。
