...
| 实现位置 | `src/signal_analysis/_numeric_energy.py::detect_signals`（参与 Cython 核心编译；`_numeric.py` 保留兼容导出） |
...
> 同一份时频图之上的**逐跳**路径见 [信号检测_跳频逐跳参数估计](信号检测_跳频逐跳参数估计.md)
> （契约 `fh_hops_v1`），它把本文档合并成一条会话记录的跳频信号拆回逐跳；
> 逐跳本身也有两条实现：传统跟踪 `hop_track_v1`，以及把判决换成网络框的 AI 通路
> `ml_detect_hops`（见 [信号检测_AI时频图检测](信号检测_AI时频图检测.md) §7），
> 两者的辐射量都在本文档的 PSD 上重测，因此可同口径并排评分。

实现分工：`_numeric_energy.py` 保存能量判决、配置与会话合并；`_numeric_common.py` 提供 STFT、噪声底、掩膜和占用频带计算。`core_api.detect_signals` 与旧 `_numeric.detect_signals` 均指向同一实现，输出契约与数值口径不变。详细模块依赖见[数值算法模块拆分与回归说明](../数值算法模块拆分与回归说明.md)。
...
| 跳频信号按"一段会话一个实例"合并统计 | 需要逐跳、逐突发的精细时间定位（改用[跳频逐跳参数估计](信号检测_跳频逐跳参数估计.md)） |
...
| `hopping` | bool | 是否由多个子带合并而来；**它不等于"这是跳频信号"**：相邻跳频信道间的缝小于一个频点时，掩膜与闭运算会把它们连成一条连续带，于是得到 `sub_bands=1`、`hopping=false`（与本契约无关的逐跳信息见[跳频逐跳参数估计](信号检测_跳频逐跳参数估计.md)） |
| `sub_bands` | int | 合并进本目标的子带数（非跳频为 1）；它是**会话合并次数，不是跳数**——一条跳频会话可以是 `hopping=true` 而任一跳的具体位置无法从本契约得到 |
...
7. **跳频会话合并是启发式判据，不是跟踪算法。**
   ...
   也没有逐跳的跳时刻/跳频点列表。需要逐跳信息时改用
   [信号检测_跳频逐跳参数估计](信号检测_跳频逐跳参数估计.md)（契约 `fh_hops_v1`）：
   它在**同一份时频图**上逐帧提取游程并做时频跟踪，能给出跳频点、跳时刻、驻留与跳速，
   而本文档这条会话记录只提供"整段链路占用了哪一段频谱"。
   *实测差别*：资产 `01588926` 在本文档口径下是**一条** `hopping=true`、`sub_bands=2` 的检测
   （带宽 93750 Hz、SNR 20.29 dB），在逐跳口径下是 **5 跳 / 1 个会话**；资产 `c801d634`
   因为相邻信道间的缝小于一个频点，被掩膜连成一条连续带，本文档口径下甚至是
   `hopping=false`、`sub_bands=1`，逐跳口径下仍有 2 跳。
...
### 7.5 跳频会话：从启发式判据到显式跟踪

- **已落地的一步**：逐帧游程 + 时频脊线跟踪（`hop_track_v1`），见
  [信号检测_跳频逐跳参数估计](信号检测_跳频逐跳参数估计.md)。它保留了本节的
  "一段会话一个实例"口径，另外给出逐跳的跳频点、跳时刻、驻留与跳速；本节剩下的
  改进空间（合并置信度、多机交错时的关联强度）在那里同样存在。
...
[信号检测_AI时频图检测](信号检测_AI时频图检测.md) §7 与
[调制识别_AI特征学习](调制识别_AI特征学习.md) §7。
