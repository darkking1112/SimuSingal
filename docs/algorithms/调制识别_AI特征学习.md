...
共享数值实现：34 维特征及固定顺序转换来自 `_numeric_modulation.py`；特征网络和原始 IQ 网络共用 `_numeric_preprocess.py` 的校验、混频、低通和抽取。`ml.amc` 保留旧特征接口供训练脚本调用，其模型训练、加载与推理职责不变；迁出的数值实现随核心编译。详细模块依赖见[数值算法模块拆分与回归说明](../数值算法模块拆分与回归说明.md)。
