...
| 特征实现 | `src/signal_analysis/_numeric_modulation.py::extract_features` |
| 启发式判定 | `src/signal_analysis/_numeric_modulation.py::classify_modulation`，标识 `heuristic_digital_analog_v1` |
...
>
> **原始 IQ 通路（`iq_waveform_v1`）没有对应的人工判据**：下面的判据全部建立在 34 维确定性特征上，
> 而那条通路不产出这些特征（网络直接从波形学），因此产品里对 IQ 结果**不显示**传统对照行，
> 界面与报告按"不适用"呈现；它的设计、契约与可改进方向见
> [调制识别_AI特征学习](调制识别_AI特征学习.md) §7。两条通路的前端（搬移零频、低通抽取）
> 共用一份实现，但分析结果**不可同口径比较**（输入信息不同）。

实现分工：34 维特征与启发式判定统一位于 `_numeric_modulation.py`；中心裁剪、混频、低通、抽取与 SNR 粗估位于 `_numeric_preprocess.py`，由特征通路和原始 IQ 通路共用。`ml.amc.extract_features`、`feature_vector` 及原常量保留兼容导出；模型训练、推理和结果组装继续留在 `ml/amc.py`。启发式直接处理 IQ，不消费 34 维向量。详细模块依赖见[数值算法模块拆分与回归说明](../数值算法模块拆分与回归说明.md)。
...
> ISI 会把 16QAM 与 64QAM 的 `c63_mag` 差别压到 2 倍以内（见 §5），
> 这正是线性判别最难的一段。
...
| `sha256` | 模型文件摘要 |
| `accuracy_in_sample` | 训练集内准确率（**不是**泛化指标，见 §5） |
