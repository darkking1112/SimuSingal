...
建议以 Python 建设离线通信实验平台，使用离散事件组织终端、卫星、业务和时隙，使用版本化模型接口接入经验证的处理模块，并提供实验管理、日志回放、统计和数据导出。

本文为通用软件架构和抽象仿真设计，不展开专用军事通信协议、具体跳频图案、同步算法、抗干扰参数或实装链路优化。对原文列出的编码、调制、同步和协议功能，定义模型位置、交付依赖及验收责任；实际能力取决于独立的模型规范与参考实现。

不能把抽象事件演示等同于完整波形仿真，也不能把离线仿真等同于实时设备运行。两者应分别列入范围和验收记录。
...
| C01～C03（用户新增） | 核心模块二进制化及可重建交付 | Cython 编译边界、开发/二进制双模式回归 | 编译清单、目标环境及源码包形式待冻结 |

模型注册表出现三个名称只说明接口存在，不能作为“支持三种编码/调制”的合格证据。每类均需要对应的可执行模型、参考输入输出及独立测试记录。
...
建议先建设 L0 软件主链路和 L1 模型适配机制，明确 L2 所需数据范围、计算资源及模型来源后实施。若合同要求完整基带波形，则 L2 为必交范围，不能以 L0/L1 替代；在缺少模型定义时，该部分应记录为待落实依赖。
...
原文没有指定操作系统、硬件或交付日期。可复用另一项目的构建工具，但不能自动继承其 Windows/麒麟双平台承诺、最低硬件配置或 2026-11-30 交付日。
...
本项目新增的是核心二进制化要求；用户 DLL/SO 算法 SDK 为电磁信号分析识别项目的明确新增范围，不自动作为本项目必交功能。
...
以下为软件框架与已有参考模型集成的建议 12～16 周安排，不包含未知专用模型从零研制。原文没有给出工期，实际计划应以模型资产清单为基础。

| 阶段 | 建议窗口 | 产出及退出条件 |
| --- | --- | --- |
| 范围与可行性 | 第 1～2 周 | 仿真层级、频率与时钟口径、模型清单、容量估算 |
| 实验引擎 | 第 3～5 周 | 场景校验、1 星 2 终端抽象事件、实验记录 |
| 模型适配 | 第 6～9 周 | 信源、处理、协议组件的参考测试与适配 |
| 工作台及回放 | 第 8～11 周，与模型集成由不同职责人员推进 | 参数编辑、运行管理、时间轴、数据及报告导出 |
| 系统验证 | 第 12～14 周 | 追踪矩阵、可重复性、容量和目标机测试 |
| 交付整改 | 第 15～16 周，按缺陷情况使用 | 发布包、源码、模型说明、用户与测试文档 |

建议投入：仿真架构/技术负责人 1 人、Python 开发 2 人、模型适配与参考验证 1～2 人、测试及部署 1 人，约 5～6 人。若与另一项目并行推进，可共用部分基础库和发布工具，但同一人员投入不能在两个排期里各按满额计算。

V1.1 要求在前两周验证核心扩展构建，在引擎开发阶段固定可编译模块边界，在系统验证阶段完成开发版/二进制版回归和无源码仓库部署；新增平台构建及维护成本需重新核算。

建议交付：源代码、桌面和批量入口、场景模板、模型注册清单、参考实验数据、结果数据包、依赖锁文件、构建脚本、需求规格说明、设计说明、测试报告、用户手册及未关闭问题清单。原文只明确“X 星仿真数据”，最终正式交付项应在需求基线中确认。

核心编译新增交付为目标平台核心扩展、编译环境清单、构建脚本及对应源码包。是否另在运行包中附带核心明文源码按用户确认执行。

## 9. 评审待办

1. 明确 L0/L1/L2 中哪些层级属于最终必交范围，是否要求实时运行。
2. 明确 1 GHz 的物理与数据含义、采样表示、仿真时长和数据保存量。
3. 提供或指定 SDPSK、其他处理模型及专用协议的版本化规范与参考输入输出。
4. 明确业务方向、语音格式与质量、缓存及业务完成判据。
5. 明确“X 星仿真数据”是输入、输出还是二者，以及格式、规模和参考来源。
6. 确定操作系统、目标硬件、模型资产可用时间及交付日期。
