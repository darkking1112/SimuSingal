...
| GUI | 七个标签页：数据分析、IQ 信号生成、信号检测、跳频参数、调制识别、算法对比、运行记录 | 仿真页面与本项目运行历史 |
...
| [src/signal_analysis/gui.py](../src/signal_analysis/gui.py) | 独立分析界面的七个标签页：数据分析（资产列表、备注、四类图形含星座图与固定时长滚动瀑布图、波形/频谱固定显示范围、概览与实时播放两种方式）、IQ 信号生成、信号检测、跳频参数、调制识别（特征清单与原始 IQ 清单两个分支）、算法对比与运行记录（含原生示例） |
| [src/signal_analysis/tasks.py](../src/signal_analysis/tasks.py) | 绑定分析 worker、提前验证项目数据目录 |
| [src/signal_analysis/services.py](../src/signal_analysis/services.py) | 生成、IQ 生成、导入、分析、detect、detect_hops、ml_detect、ml_detect_hops、amc_classify、amc_iq_classify 与原生复制业务，负责附加生成器真值与检测基线，不分派仿真 |
...
| [tests/analysis/test_detect_hops.py](../tests/analysis/test_detect_hops.py) | 跳频逐跳参数估计（54 项）：配置校验、游程/跟踪几何、驻留重测、细网格频带复算、会话归并、可分辨性与 `reason`、真值并集与会话级一致、服务层“不适用”与 CLI/GUI/报告路由 |
...
| [tests/analysis/test_gui_analysis.py](../tests/analysis/test_gui_analysis.py) | 七个标签页、生成/分析、图形、星座图与手动判定、固定显示范围、实时播放与滚动瀑布图、备注、报告、IQ 生成页、检测工作流及算法对比页 |
