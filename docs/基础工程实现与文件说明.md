...
| [src/common/gui.py](../src/common/gui.py) | 共用窗口外壳、主题、任务状态、取消与报告导出；**不创建任何标签页**，标签页由各项目在自己的 `MainWindow.__init__` 中注册并排序，并提供 `result_status()` 钩子供只读页面改写完成文案；底部两行状态栏——`self.status` 写任务状态，`self.status_detail`（`ElidedLabel`，超长按中间省略、完整文本仍可 `text()` 取到）经 `set_status_detail()` 写业务摘要，空文本即隐藏 |
...
| [src/signal_analysis/gui.py](../src/signal_analysis/gui.py) | 独立分析界面的八个标签页（在 `MainWindow.__init__` 的页面注册表 `pages` 中按顺序注册并登记 `self.page_index`；此后取下标一律用 `self._page_index("页面名")`、`tab_results` 也按页面名做键，任何地方不写死数字）：数据分析（资产列表、备注、四类图形含星座图与固定时长滚动瀑布图、波形/频谱固定显示范围、概览与实时播放两种方式；侧栏采样率仅用于导入）、IQ 信号生成（多信号合成、带限噪声与“数学双音演示”入口，演示点数 = 页内采样率 × 持续时间）、信号检测、调制识别（特征清单与原始 IQ 清单两个分支）、算法对比、跳频参数、数据管理（工作区与额外目录容量盘点、索引一致性核对、白名单安全清理，见[数据管理页面](数据管理页面.md)）与运行记录（`build_history`/`refresh_history`/`open_history`，含原生示例）；数据分析/信号检测/跳频参数三页共用同一“频率显示”语义（自动按 PSD 镜像判实数记录、双边、仅正频率），同页内的谱线、时频图与检测框/逐跳框孄用同一频率子集，时频图按“行 = 时间、列 = 频率”绘制；在侧栏选中数据资产时把文件名、**相对工作目录**的位置（不重复写工作目录本身）、文件大小（复采样数 @ 采样率 · 时长）、`资产：`（工作区 NPY（complex64）← 内置生成 xxx / 导入 NPY|CSV|交织 IQ 二进制|SigMF）与 `导出：`（现扫 `exports/` 下 `<asset_id>.*`，按后缀还原格式名；SigMF 成对文件只计一条，`.bin` 按字节数与复采样数反推 int16/float32，没有导出物写“无”）写入状态栏第二行，文件被外部删掉时显式提示“文件缺失”，取消选择则清空该行 |
...
| [src/signal_analysis/_numeric.py](../src/signal_analysis/_numeric.py) | 旧导入路径的显式兼容导出，不包含算法实现；同样参与核心编译 |
| [src/signal_analysis/_numeric_common.py](../src/signal_analysis/_numeric_common.py) | 公共校验、STFT/PSD、噪声本底、掩膜、占用区间与共享常量；可编译 |
| [src/signal_analysis/_numeric_analysis.py](../src/signal_analysis/_numeric_analysis.py) | 统计摘要、波形／频谱／时频／星座数组与单帧频谱行；可编译 |
| [src/signal_analysis/_numeric_iqgen.py](../src/signal_analysis/_numeric_iqgen.py) | 九种样式 IQ 生成、信号规划、带限噪声、双音及逐跳采样边界；可编译 |
| [src/signal_analysis/_numeric_preprocess.py](../src/signal_analysis/_numeric_preprocess.py) | AMC 与原始 IQ 分类共用的中心裁剪、混频、低通、抽取及 SNR 粗估；可编译 |
| [src/signal_analysis/_numeric_modulation.py](../src/signal_analysis/_numeric_modulation.py) | 34 维传统特征、固定顺序向量转换及数字／模拟启发式判定；可编译 |
| [src/signal_analysis/_numeric_energy.py](../src/signal_analysis/_numeric_energy.py) | 会话级能量检测、双门限频带量测、时间支撑与会话合并；可编译 |
| [src/signal_analysis/_numeric_hops.py](../src/signal_analysis/_numeric_hops.py) | 逐跳配置、时频脊线跟踪、频带复测、驻留量测、会话与可分辨性判定；可编译 |
...
| [src/signal_analysis/ml/amc.py](../src/signal_analysis/ml/amc.py) | A09 六类模型训练、加载、推理与结果组装；重导出迁至 `_numeric_modulation`／`_numeric_preprocess` 的 34 维特征和预处理旧接口；模型契约、低信噪比提示与 `pending` 待确认项不变 |
...
职责划分、依赖关系、兼容入口、中文注释规范与验证记录见[数值算法模块拆分与回归说明](数值算法模块拆分与回归说明.md)。构建与验收共用 [packaging/numeric_core.json](../packaging/numeric_core.json)；[scripts/check_numeric_equivalence.py](../scripts/check_numeric_equivalence.py) 保存和核对同环境数值基线。
